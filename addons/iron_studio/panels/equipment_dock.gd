@tool
extends Control
## 装备编辑器 —— 武器/装备创建、融合、吞噬、词条配置

const DATA_DIR := "res://data/weapons"
const WeaponDataScript := preload("res://core/data/weapon_data.gd")
...
func _refresh() -> void:
	...
	while f != "":
		if f.ends_with(".tres"):
			_list.add_item(f.trim_suffix(".tres"))
		f = dir.get_next()
	dir.list_dir_end()
...
func _on_new_pressed() -> void:
	...
	w.weapon_id = n.to_lower(); w.weapon_name = n
	w.rarity = _rarity.get_item_text(_rarity.selected)
	w.base_atk = int(_atk.value)
	w.fusion_level = int(_fusion.value)
	w.max_fusion = int(_max_fusion.value)
	w.fusion_atk_growth = _growth.value
	DirAccess.make_dir_recursive_absolute(DATA_DIR)
	ResourceSaver.save(w, DATA_DIR + "/" + n.to_lower() + ".tres")
	_show("已创建: %s (ATK:%d 融合:%d/%d)" % [n, w.current_atk(), w.fusion_level, w.max_fusion])
	_refresh()
...
func _on_load_pressed() -> void:
	...
	_atk.value = w.base_atk
	_fusion.value = w.fusion_level
	_max_fusion.value = w.max_fusion
	_growth.value = w.fusion_atk_growth
...
func _on_save_pressed() -> void:
	...
	w.fusion_atk_growth = _growth.value
	ResourceSaver.save(w, DATA_DIR + "/" + n.to_lower() + ".tres")
	_show("已保存: %s (ATK:%d)" % [n, w.current_atk()])
	_refresh()
