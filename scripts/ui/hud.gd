...
func _build_ui() -> void:
	...
	gold_label.position = Vector2(16, 276)
	gold_label.add_theme_font_size_override("font_size", 18)
	add_child(gold_label)

	inventory_label = RichTextLabel.new()
	inventory_label.position = Vector2(16, 312)
...
func _on_stats_changed() -> void:
	...
	stats_label.text = (
		"HP %d/%d    攻击 %.0f    防御 %.0f\n" % [int(s.hp), int(s.max_hp), s.get_value(AttributeSystem.Stat.ATK), s.get_value(AttributeSystem.Stat.DEF)]
