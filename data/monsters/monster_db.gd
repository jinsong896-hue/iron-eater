...
##
## 机制实现现状：59 个非 none 机制标记中 **58 个已实装**，
## 仅 `boss_skill` 仍是纯数据登记——分册 9-8 只说「继承最终 Boss 基础技能 1 个」，
## **没指明是哪一个**，且最终 Boss 本身（破坏神完全体·终焉之翼）在 `boss_db.gd`
## 是四机制组合而非"基础技能"列表，无从推导（见 docs/progress/PROGRESS_20260919.md 第三节）。
...
# 机制标记 none = 无；其余标记全部已实装，未实装的 1 个（boss_skill）见文件头
# ============================================================
const BASE_TYPES := [
	# ① 缓慢近身（僵尸类）—— 分册 4.1
...
## 走群体模拟（CrowdSim）的基础怪原型 key —— 准入条件见 `_build_base_monsters`。
##
## 目前只有这三支：缓慢近身（僵尸）、普通速度近身（狂暴囚犯）、突进近战（猎犬）。
## 都是「冲上来贴脸打」的纯近战，行为可被模拟核完整表达。
## 弓手/哨兵/鬼火（远程）、蝙蝠（飞行）、巨鼠（大体积）、幽灵（高闪避）
## 暂不搬——理由分别是核只有近身判定、核无高度、占位网格尺寸固定、闪避未进核。
const SWARM_BASE_KEYS := ["zombie", "berserker", "hound"]
	...
	for row in BASE_TYPES:
		...
		for i in stages.size():
			...
			_monsters[id] = {
				...
				"base_key": key,
				"is_elite": false,
				# 走群体模拟（CrowdSim）与否——路由判据见 room_controller._should_use_crowd。
				#
				# **为什么是数据标记而不是全局开关**：搬去模拟核的怪会丢掉节点侧的
				# 一整套行为（AI 状态机、朝向、机制、词缀、命中特效），只保留
				# "移动 + 碰撞 + 渲染 + 近身攻击"。所以只能搬**行为最简单**的那批，
				# 逐个标记，不能一刀切。
				#
				# 准入条件（三条同时满足才标）：
				#   ① `special` 只有 `{"mechanic": ...}` 登记项——即无引擎已实现的机制。
				#      `death_poison` 这类带真实现的会被 `_should_use_crowd` 拦下，
				#      标了也不生效，但留个假标记会误导后来人，故不标。
				#   ② 近战（`ai` 为 melee / rusher）——模拟核的攻击判定是**近身**的
				#      （`set_attack_params(1.4, 1.2, 8.0)`），远程怪搬过去会变成
				#      贴到脸上才打，手感完全不对。
				#   ③ `scale` 接近 1.0——群体单位当前是**固定尺寸的占位胶囊**
				#      （`_setup_multimesh` 的 CapsuleMesh，见待办「换掉占位网格」），
				#      巨鼠这类大体积怪搬过去会视觉缩水，等网格换完再评估。
				#
				# 据此当前覆盖：僵尸、狂暴囚犯、猎犬（phase 1~4）。
				# 毒瘴僵尸虽也是近战，但 `death_poison` 有真实现，不标。
				"swarm": key in SWARM_BASE_KEYS,
				"is_unique": false,
			}
