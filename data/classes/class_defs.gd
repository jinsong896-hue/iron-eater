...
##
## ⚠ 当前进度：**5 职业 × 25 形态 × 40 技能全部实装**，31 个形态机制全部接线
##（`test_framework.gd` 断言 `pending_classes()` 为空、`EXPECTED_UNIMPLEMENTED` 恒为空）。
...
const CLASSES := {
	...
	"hunter": {
		...
		"resource": "hunter",
		"desc": "命中/暴击积攒魔力，自动回蓝。走 A 拉扯，暴击压制。",
		"forms": [
			# ---------- 初始：追猎者 ----------
			{
				"name": "追猎者",
