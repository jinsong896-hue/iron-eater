...
- 商店出售装备已落地（白~蓝保底 / 2 级解锁紫装 / 3 级 9 折）；消耗品体系待策划
...
- 方向键（↑↓←→）切换朝向并攻击（扇形判定）
- `1`~`4` 技能　`E` 交互　`F` 吞噬附近掉落物　`I` 背包　`ESC` 暂停
- `` ` `` 调试控制台　`F1` 调试面板

**手柄**（Xbox 布局，映射写在 `project.godot`）

| 操作 | 手柄 | 操作 | 手柄 |
|---|---|---|---|
| 移动 | 左摇杆 / 十字键 | 攻击（八方向） | 右摇杆 |
| 交互 | A | 翻滚 | B |
| 技能 1~4 | X / Y / LB / RB | 冲刺 | 左摇杆按下 |
| 吞噬 | 右摇杆按下 | 背包 | BACK |
| 暂停 | START | | |

> 攻击走**右摇杆**而非 ABXY：本项目攻击是八方向扇形判定，
> 摇杆能表达方向，按钮不能。若要改用 ABXY，需先给攻击加
> 「无方向时取朝向」的兜底语义。
...
3. 消耗品体系（炸弹破墙）——容器 `ConsumableInventory` 已在，只缺炸弹道具，
   但其规格策划书自相矛盾（分册 186 行说"无消耗品复活"，722 行又把复活币
   列为消耗品奖励），需策划先定
4. 等策划给数再动：元素武器表 / 红装逐件机制表 / 稀有度具名后缀 / 红装碎片规则
